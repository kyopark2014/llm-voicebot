...
아래에서는 Typescript를 이용하여 인프라를 정의하는 방법에 대해 설명합니다.
...
Deploy할 때에 /html 폴더의 내용을 S3 bucket에 복사합니다.
...
Redis를 위한 Subnet과 Security Group을 생성합니다.
...
Websocket을 처리하는 API Gateway를 생성합니다.
...
Lambda (greeting)을 생성합니다. (시나리오 변경하면서 사용하지는 않음. 백업용)
...
Lambda (reading)을 생성합니다. (multimodal 용)
...
Lambda를 위한 Security group을 생성합니다. 
...
Voice session을 설정하기 위한 Lambda (voice provisioning)을 생성합니다. 
